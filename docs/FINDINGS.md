...
- [`v12_nvidia_recipe_2026_05_24.md`](findings/v12_nvidia_recipe_2026_05_24.md) — Session 5: aligned conversion with NVIDIA's V3.2-NVFP4 reference recipe — ZERO mtp transformation, all `mtp.*` tensors byte-passthrough from native. Built v12 artifact (913 GB). Discovered and FIXED a bug in our `_mtp_block_is_quantized_on_disk` (it scanned for post-rename `.weight_scale_inv` etc. but native artifacts ship with raw `.scale`). Post-fix the loader successfully registers `e_proj.weight_scale_inv` (39 MTP params instead of 29). Deployment blocked by silent worker crash that affects v0.2 too on the rebuilt mainline — likely flashinfer 0.6.8 → 0.6.11 ABI regression.
